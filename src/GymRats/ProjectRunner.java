...
public class ProjectRunner {
	...
	public static void main(String[] args) throws ClassNotFoundException {
		/*
		 * UserController user = new UserController(connectionUrl, dbUsername,
		 * dbPassword); ExerciseController exercise = new
		 * ExerciseController(connectionUrl, dbUsername, dbPassword); CommentController
		 * comment = new CommentController(connectionUrl, dbUsername, dbPassword);
		 * WorkoutController workout = new WorkoutController(connectionUrl, dbUsername,
		 * dbPassword); UserWorkoutController userWorkout = new
		 * UserWorkoutController(connectionUrl, dbUsername, dbPassword);
		 * BodyProgressController bodyProgress = new
		 * BodyProgressController(connectionUrl, dbUsername, dbPassword);
		 * 
		 * System.out.println("\nMax: " + userWorkout.getMax(1, "deadlift") + "\n");
		 * ArrayList<UserWorkout> unordered = userWorkout.getAllForExcerciseName(1,
		 * "deadlift"); System.out.println("\nUnordered Deadlift: " + unordered + "\n");
		 * System.out.println("\nInorder Deadlift: " +
		 * userWorkout.sortUserWorkouts(unordered) + "\n"); // The code below will get
		 * all of the record lifts for deadlift from all users, // sort them, and list
		 * the top 5
		 * 
